/*

Copyright 2023 Keyfactor
Licensed under the Apache License, Version 2.0 (the License); you may
not use this file except in compliance with the License.  You may obtain a
copy of the License at http://www.apache.org/licenses/LICENSE-2.0.  Unless
required by applicable law or agreed to in writing, software distributed
under the License is distributed on an AS IS BASIS, WITHOUT WARRANTIES
OR CONDITIONS OF ANY KIND, either express or implied. See the License for
the specific language governing permissions and limitations under the
License.

Keyfactor API Reference and Utility

<p>This page provides a utility through which the Keyfactor API endpoints can be called and results returned.                                                           It is intended to be used primarily for validation, testing and workflow development.                                                           It also serves secondarily as documentation for the API.</p>                                                          <p>If you would like to view documentation containing details on the Keyfactor API and endpoints,                                                           please refer to the Web API section of the Keyfactor Command documentation.</p>

API version: 1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package command

import (
	"encoding/json"
)

// checks if the ModelsCertificateStoreServer type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ModelsCertificateStoreServer{}

// ModelsCertificateStoreServer struct for ModelsCertificateStoreServer
type ModelsCertificateStoreServer struct {
	Id         *int32                 `json:"id,omitempty"`
	Username   *ModelsKeyfactorSecret `json:"username,omitempty"`
	Password   *ModelsKeyfactorSecret `json:"password,omitempty"`
	UseSSL     *bool                  `json:"useSSL,omitempty"`
	ServerType *int32                 `json:"serverType,omitempty"`
	Name       NullableString         `json:"name,omitempty"`
}

// NewModelsCertificateStoreServer instantiates a new ModelsCertificateStoreServer object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewModelsCertificateStoreServer() *ModelsCertificateStoreServer {
	this := ModelsCertificateStoreServer{}
	return &this
}

// NewModelsCertificateStoreServerWithDefaults instantiates a new ModelsCertificateStoreServer object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewModelsCertificateStoreServerWithDefaults() *ModelsCertificateStoreServer {
	this := ModelsCertificateStoreServer{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *ModelsCertificateStoreServer) GetId() int32 {
	if o == nil || isNil(o.Id) {
		var ret int32
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ModelsCertificateStoreServer) GetIdOk() (*int32, bool) {
	if o == nil || isNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *ModelsCertificateStoreServer) HasId() bool {
	if o != nil && !isNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given int32 and assigns it to the Id field.
func (o *ModelsCertificateStoreServer) SetId(v int32) {
	o.Id = &v
}

// GetUsername returns the Username field value if set, zero value otherwise.
func (o *ModelsCertificateStoreServer) GetUsername() ModelsKeyfactorSecret {
	if o == nil || isNil(o.Username) {
		var ret ModelsKeyfactorSecret
		return ret
	}
	return *o.Username
}

// GetUsernameOk returns a tuple with the Username field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ModelsCertificateStoreServer) GetUsernameOk() (*ModelsKeyfactorSecret, bool) {
	if o == nil || isNil(o.Username) {
		return nil, false
	}
	return o.Username, true
}

// HasUsername returns a boolean if a field has been set.
func (o *ModelsCertificateStoreServer) HasUsername() bool {
	if o != nil && !isNil(o.Username) {
		return true
	}

	return false
}

// SetUsername gets a reference to the given ModelsKeyfactorSecret and assigns it to the Username field.
func (o *ModelsCertificateStoreServer) SetUsername(v ModelsKeyfactorSecret) {
	o.Username = &v
}

// GetPassword returns the Password field value if set, zero value otherwise.
func (o *ModelsCertificateStoreServer) GetPassword() ModelsKeyfactorSecret {
	if o == nil || isNil(o.Password) {
		var ret ModelsKeyfactorSecret
		return ret
	}
	return *o.Password
}

// GetPasswordOk returns a tuple with the Password field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ModelsCertificateStoreServer) GetPasswordOk() (*ModelsKeyfactorSecret, bool) {
	if o == nil || isNil(o.Password) {
		return nil, false
	}
	return o.Password, true
}

// HasPassword returns a boolean if a field has been set.
func (o *ModelsCertificateStoreServer) HasPassword() bool {
	if o != nil && !isNil(o.Password) {
		return true
	}

	return false
}

// SetPassword gets a reference to the given ModelsKeyfactorSecret and assigns it to the Password field.
func (o *ModelsCertificateStoreServer) SetPassword(v ModelsKeyfactorSecret) {
	o.Password = &v
}

// GetUseSSL returns the UseSSL field value if set, zero value otherwise.
func (o *ModelsCertificateStoreServer) GetUseSSL() bool {
	if o == nil || isNil(o.UseSSL) {
		var ret bool
		return ret
	}
	return *o.UseSSL
}

// GetUseSSLOk returns a tuple with the UseSSL field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ModelsCertificateStoreServer) GetUseSSLOk() (*bool, bool) {
	if o == nil || isNil(o.UseSSL) {
		return nil, false
	}
	return o.UseSSL, true
}

// HasUseSSL returns a boolean if a field has been set.
func (o *ModelsCertificateStoreServer) HasUseSSL() bool {
	if o != nil && !isNil(o.UseSSL) {
		return true
	}

	return false
}

// SetUseSSL gets a reference to the given bool and assigns it to the UseSSL field.
func (o *ModelsCertificateStoreServer) SetUseSSL(v bool) {
	o.UseSSL = &v
}

// GetServerType returns the ServerType field value if set, zero value otherwise.
func (o *ModelsCertificateStoreServer) GetServerType() int32 {
	if o == nil || isNil(o.ServerType) {
		var ret int32
		return ret
	}
	return *o.ServerType
}

// GetServerTypeOk returns a tuple with the ServerType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ModelsCertificateStoreServer) GetServerTypeOk() (*int32, bool) {
	if o == nil || isNil(o.ServerType) {
		return nil, false
	}
	return o.ServerType, true
}

// HasServerType returns a boolean if a field has been set.
func (o *ModelsCertificateStoreServer) HasServerType() bool {
	if o != nil && !isNil(o.ServerType) {
		return true
	}

	return false
}

// SetServerType gets a reference to the given int32 and assigns it to the ServerType field.
func (o *ModelsCertificateStoreServer) SetServerType(v int32) {
	o.ServerType = &v
}

// GetName returns the Name field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ModelsCertificateStoreServer) GetName() string {
	if o == nil || isNil(o.Name.Get()) {
		var ret string
		return ret
	}
	return *o.Name.Get()
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ModelsCertificateStoreServer) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Name.Get(), o.Name.IsSet()
}

// HasName returns a boolean if a field has been set.
func (o *ModelsCertificateStoreServer) HasName() bool {
	if o != nil && o.Name.IsSet() {
		return true
	}

	return false
}

// SetName gets a reference to the given NullableString and assigns it to the Name field.
func (o *ModelsCertificateStoreServer) SetName(v string) {
	o.Name.Set(&v)
}

// SetNameNil sets the value for Name to be an explicit nil
func (o *ModelsCertificateStoreServer) SetNameNil() {
	o.Name.Set(nil)
}

// UnsetName ensures that no value is present for Name, not even an explicit nil
func (o *ModelsCertificateStoreServer) UnsetName() {
	o.Name.Unset()
}

func (o ModelsCertificateStoreServer) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ModelsCertificateStoreServer) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !isNil(o.Username) {
		toSerialize["username"] = o.Username
	}
	if !isNil(o.Password) {
		toSerialize["password"] = o.Password
	}
	if !isNil(o.UseSSL) {
		toSerialize["useSSL"] = o.UseSSL
	}
	if !isNil(o.ServerType) {
		toSerialize["serverType"] = o.ServerType
	}
	if o.Name.IsSet() {
		toSerialize["name"] = o.Name.Get()
	}
	return toSerialize, nil
}

type NullableModelsCertificateStoreServer struct {
	value *ModelsCertificateStoreServer
	isSet bool
}

func (v NullableModelsCertificateStoreServer) Get() *ModelsCertificateStoreServer {
	return v.value
}

func (v *NullableModelsCertificateStoreServer) Set(val *ModelsCertificateStoreServer) {
	v.value = val
	v.isSet = true
}

func (v NullableModelsCertificateStoreServer) IsSet() bool {
	return v.isSet
}

func (v *NullableModelsCertificateStoreServer) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableModelsCertificateStoreServer(val *ModelsCertificateStoreServer) *NullableModelsCertificateStoreServer {
	return &NullableModelsCertificateStoreServer{value: val, isSet: true}
}

func (v NullableModelsCertificateStoreServer) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableModelsCertificateStoreServer) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
